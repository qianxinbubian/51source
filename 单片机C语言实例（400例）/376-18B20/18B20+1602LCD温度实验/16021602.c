...
/*******************************************************************/
 Disp_Temperature()
{ // 温度高8位SSSS Sb1b2b3    温度低8位b4b5b6b7 x1x2x3x4
 //高8位前5位是符号位，紧接着b1b2b3b4b5b6b7是整数部分，x1x2x3x4位小数部分     
  display[4]=temp_data[0]&0x0f ;//取温度低8位的低4位（小数位）
  display[0]=ditab[display[4]]+0x30 ;//查表得小数位的值	//+0x30是转换成IC码
  
  display[4]=((temp_data[0]&0xf0)>>4)|((temp_data[1]&0x0f)<<4) ;//整数部分取值
  display[3]=display[4]/100+0x30 ; //求商得百位值（显示）
  display[1]=display[4]%100 ; //求余得到十位个位数据
  display[2]=display[1]/10+0x30 ;//得到十位数据（显示）
  display[1]=display[1]%10+0x30 ;//得到个位数据（显示）
